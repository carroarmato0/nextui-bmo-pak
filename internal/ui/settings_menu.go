...
func (m *SettingsMenu) Overlay() OverlayState {
	...
	items := []OverlayItem{
		...
		{Code: "stt_key", Label: providerKeyLabel("STT", m.cfg.STT.APIKey, m.editing && m.editingKind == "stt", m.draft), Selected: strings.TrimSpace(m.cfg.STT.APIKey) != "", Focused: m.focus == 1 && !m.editing},
		{Code: "chat_key", Label: providerKeyLabel("CHAT", m.cfg.Chat.APIKey, m.editing && m.editingKind == "chat", m.draft), Selected: strings.TrimSpace(m.cfg.Chat.APIKey) != "", Focused: m.focus == 2 && !m.editing},
		{Code: "tts_key", Label: providerKeyLabel("TTS", m.cfg.TTS.APIKey, m.editing && m.editingKind == "tts", m.draft), Selected: strings.TrimSpace(m.cfg.TTS.APIKey) != "", Focused: m.focus == 3 && !m.editing},
	}
	subtitle := []string{"SETTINGS: API KEYS AND MODE", "ENTER TO EDIT KEY, SPACE TO TOGGLE MODE"}
	footer := "START TO SAVE"
	if m.editing {
...
func (m *SettingsMenu) BeginAPIKeyEdit(kind string) error {
	if m == nil { return fmt.Errorf("nil settings menu") }
...
func (m *SettingsMenu) InsertText(text string) {
	if m == nil || !m.editing { return }
	m.draft += text
}
func (m *SettingsMenu) Backspace() {
	if m == nil || !m.editing || m.draft == "" { return }
	r := []rune(m.draft)
	m.draft = string(r[:len(r)-1])
}
func (m *SettingsMenu) SubmitEdit() error {
	if m == nil { return fmt.Errorf("nil settings menu") }
	if !m.editing { return nil }
	if err := m.SetAPIKey(m.editingKind, m.draft); err != nil { return err }
	m.editing = false
	m.editingKind = ""
	m.draft = ""
	return nil
...
func (m *SettingsMenu) currentAPIKey(kind string) string {
	switch kind {
	case "stt": return m.cfg.STT.APIKey
	case "chat": return m.cfg.Chat.APIKey
	case "tts": return m.cfg.TTS.APIKey
	default: return ""
	}
}
